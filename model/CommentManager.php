...
class CommentManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $req = $db->query('SELECT username, profile_picture, user_id, comment, is_valid, post_id, DATE_FORMAT(comment_date, \'%d/%m/%Y à %Hh%i\') AS comment_date_fr FROM comments LEFT JOIN user ON comments.user_id = user.id WHERE is_valid = 1 ORDER BY comment_date DESC');
